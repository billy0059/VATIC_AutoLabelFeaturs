...
function ui_setup(job)
{
    ...
    for (var frame in doubtFrame){
        //console.log(frame);
        //doubtButtonStatement += "<input type = 'button' class =\'" + appearFrame + "\' id = \'" + buttonID + "\' value = \'"+ objectName + " " + String(objectID) + "\'>"+ "</input>    ";
        var value = "";
        for (pathNameIndex in doubtFrame[frame]){
            if(value != "") {
                value += ", ";
            }
            value += doubtFrame[frame][pathNameIndex];
        }
        //console.log(value);
        doubtButtonStatement += "<div class=\"doubt button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary\" id=" + frame.toString() + " value = '"+ value + "'>" + "Frame " + frame.toString() + "</div>";
    }
    /*for (var i =0; i<(fistAppearLines.length - 1); i++){
        ...
        appearFrame = fistAppearLines[i].split(' ')[2];
        buttonID = "jumpTo" + String(objectID);
        appendStr = "<input type = 'button' class =\'" + appearFrame + "\' id = \'" + buttonID + "\' value = \'"+ objectName + " " + String(objectID) + "\'>"+ "</input>    ";
        buttonStatement += appendStr;
